...
class NotesDatabaseHelper(context: Context) :
    ...
    companion object {
        ...
        private const val TABLE_NAME = "remainders"
    ...
    fun insertReminder(note: Note): Long {
    ...
    fun getAllReminders(): MutableList<Note> {
        val remindersList = mutableListOf<Note>()
    ...
    fun updateReminder(note: Note): Int {
        val db = writableDatabase
        val values = ContentValues().apply {
            put(COLUMN_TITLE, note.title)
            put(COLUMN_CONTENT, note.content)
